<template>
  <div>
    <a-divider
      ...
      style="margin: 0; border-color: #e8e8e8"
    />
    <!-- Main Sidebar -->
      ...
      <a-layout-header>
        <a-row type="flex">
          ...
          <a-col :span="24" :md="18" class="header-control">
            ...
            <router-link v-if="!userName" to="/login" class="btn-sign-in">
  ...
  <a-dropdown v-else placement="bottomRight" :trigger="['click']">
    <a class="btn-user" style="display: flex; align-items: center;">
...
export default {
  ...
  methods: {
    ...
    async fetchUserInfo() {
      try {
        console.log('Fetching user info...', axiosInstance); // 检查 axiosInstance 是否存在
